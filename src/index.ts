...
app.post("/invocations", async (req, res) => {
  ...
  if (!body.prompt || typeof body.prompt !== "string") {
    res.status(400).json({ error: "Request body must contain a 'prompt' string field" });
    return;
  }

  // agentName 未指定時はコーディネーター (チームエージェント) を使用
  const agentName: AgentName =
    isValidAgent(body.agentName) ? body.agentName : "coordinatorAgent";
...
app.listen(PORT, () => {
  ...
  console.log(`  Team:    POST http://localhost:${PORT}/invocations  (coordinatorAgent)`);
  console.log(
